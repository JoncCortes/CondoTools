...
import { routePermissionMap, staticMenuByRole } from '../constants/roles'
...
export function AppLayout() {
  ...
  const { user, permissions, clearAuth, activeCondominiumId, setActiveCondominium } = useAuthStore()
  ...
  }, [user?.role, activeCondominiumId])

  const allowedLinks = useMemo(() => {
    const raw = menu?.menu_items?.length ? menu.menu_items.map((x) => x.path) : (staticMenuByRole[user?.role] || [])
