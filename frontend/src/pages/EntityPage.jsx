...
export function EntityPage({ config }) {
  ...
  const { user, permissions, activeCondominiumId } = useAuthStore()
  ...
  const canCreate = !config.permissions?.create || user?.role === 'PLATFORM_ADMIN' || permissions.includes(config.permissions.create)
  const canUpdate = !config.permissions?.update || user?.role === 'PLATFORM_ADMIN' || permissions.includes(config.permissions.update)
  const canDelete = !config.permissions?.delete || user?.role === 'PLATFORM_ADMIN' || permissions.includes(config.permissions.delete)

  return (
    <div>
      <div className="page-head">
        <h1>{config.title}</h1>
        {canCreate ? <button onClick={() => { setCreateOpen(true); setFormValues({}) }}>Criar / Registrar</button> : null}
      </div>
        ...
        {!editing && selected && (
          <div>
            ...
            <div className="row-actions">
              {canUpdate ? <button onClick={() => setEditing(true)}>Editar</button> : null}
              {canDelete ? <button className="danger" onClick={async () => { const ok = await remove(selected.id); if (ok) setDetailOpen(false) }}>Excluir</button> : null}
            </div>
          </div>
        )}
        {editing && (
